...
/*********************************************************************
    ...
    Description:
       ...
       This file contains the ZCL Security and Safety definitions.
...
typedef void (*zclSS_ChangeNotification_t)( uint8 zoneStatus, uint8 extendedStatus );
...
typedef void (*zclSS_EnrollRequest_t)( afAddrType_t *srcAddr, uint8 zoneID, 
...
typedef void (*zclSS_EnrollResponse_t)( uint8 responseCode,uint8 zoneID );
...
//  numberOfZones - numberOfZones - number of zones ( one byte)
//  bypassBuf - zone IDs array of 256 entries one byte each
typedef void (*zclSS_ACE_Bypass_t)( uint8 numberOfZones, uint8 *bypassBuf );
...
typedef void (*zclSS_ACE_GetZoneInformationResponse_t)( uint8 zoneID, uint16 zoneType, uint8 *ieeeAddr );

// This callback is called to process a Start Warning command
//  warnings - bitfiels ( one byte )
//  duration  - in seconds
typedef void (*zclSS_WD_StartWarning_t)( warning_t warnings, uint16 duration );
 ...
 *  Use like:
 *      ZStatus_t zclSS_Send_IAS_ACE_EmergencyCmd( uint16 srcEP, afAddrType_t *dstAddr, uint8 seqNum );
 */
#define zclSS_Send_IAS_ACE_EmergencyCmd(a,b,c) zcl_SendCommand( (a), (b), ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ACE,\
                                                      TRUE, COMMAND_SS_IAS_ACE_EMERGENCY, TRUE,\
                                                      ZCL_FRAME_CLIENT_SERVER_DIR, 0, (c), 0, NULL )

/*
 *  Send a Fire Cmd  ( IAS ACE Cluster )
 *  Use like:
 *      ZStatus_t zclSS_Send_IAS_ACE_FireCmd( uint16 srcEP, afAddrType_t *dstAddr, uint8 seqNum );
 */
#define zclSS_Send_IAS_ACE_FireCmd(a,b,c) zcl_SendCommand( (a), (b), ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ACE,\
 ...
 *  Use like:
 *      ZStatus_t zclSS_Send_IAS_ACE_PanicCmd( uint16 srcEP, afAddrType_t *dstAddr, uint8 seqNum );
 */
#define zclSS_Send_IAS_ACE_PanicCmd(a,b,c) zcl_SendCommand( (a), (b), ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ACE,\
 ...
 *  Use like:
 *      ZStatus_t zclSS_Send_IAS_ACE_GetZoneIDMapCmd( uint16 srcEP, afAddrType_t *dstAddr, uint8 seqNum );
 */
#define zclSS_Send_IAS_ACE_GetZoneIDMapCmd(a,b,c) zcl_SendCommand( (a), (b), ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ACE,\
...
extern ZStatus_t zclSS_Send_IAS_ACE_BypassCmd( uint8 srcEP, afAddrType_t *dstAddr,
                             uint8 numberOfZones, uint8 *bypassBuf, uint8 seqNum );
...
extern ZStatus_t zclSS_Send_IAS_ACE_GetZoneInformationResponseCmd( uint8 srcEP, afAddrType_t *dstAddr,
                                                                   uint8 zoneID, uint16 zoneType, 
                                                                   uint8 *ieeeAddress, uint8 seqNum );
#endif // ZCL_ACE
