/*******************************************************************************
    ...
    Zigbee Cluster Library - Security and Safety ( SS )
 ...
 * LOCAL FUNCTIONS
 */
static ZStatus_t zclSS_HdlIncoming( zclIncoming_t *msg, uint16 logicalClusterID );
...
static void zclSS_ProcessInCmd_ZoneStatus_EnrollRequest( zclIncoming_t *pInMsg );
...
static void zclSS_ProcessInCmd_ACE_Arm( zclIncoming_t *pInMsg );
...
static void zclSS_ProcessInCmd_ACE_GetZoneIDMap( zclIncoming_t *pInMsg );
static void zclSS_ProcessInCmd_ACE_GetZoneInformation( zclIncoming_t *pInMsg );
...
ZStatus_t zclSS_IAS_Send_ZoneStatusChangeNotificationCmd( uint8 srcEP, afAddrType_t *dstAddr,
                                       uint8 zoneStatus, uint8 extendedStatus, uint8 seqNum )
{
  uint8 buf[2];
  buf[0] = zoneStatus;
  buf[1] = extendedStatus;
...
ZStatus_t zclSS_IAS_Send_ZoneStatusEnrollRequestCmd( uint8 srcEP, afAddrType_t *dstAddr,
                                uint16 zoneType, uint16 manufacturerCode, uint8 seqNum )
{
  uint8 buf[4];
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ZONE,
                          ...
                          ZCL_FRAME_SERVER_CLIENT_DIR, 0, seqNum, 4, buf );
}
...
ZStatus_t zclSS_IAS_Send_ZoneStatusEnrollResponseCmd( uint8 srcEP, afAddrType_t *dstAddr,
                                         uint8 responseCode, uint8 zoneID, uint8 seqNum )
{
  uint8 buf[2];
                                     ...
                                     uint8 armMode, uint8 seqNum )
{
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ACE,
                          ...
                          ZCL_FRAME_CLIENT_SERVER_DIR, 0, seqNum, 1, buf );
}
...
ZStatus_t zclSS_Send_IAS_ACE_ArmResponse( uint8 srcEP, afAddrType_t *dstAddr,
                                          uint8 armNotification, uint8 seqNum )
{
  uint8 buf[1];
  ...
  return zcl_SendCommand( srcEP, dstAddr, ZCL_SS_LOGICAL_CLUSTER_ID_IAS_ACE,
                          ...
                          ZCL_FRAME_SERVER_CLIENT_DIR, 0, seqNum, 1, buf );
}
...
ZStatus_t zclSS_Send_IAS_ACE_GetZoneInformationResponseCmd( uint8 srcEP, afAddrType_t *dstAddr, 
                              uint8 zoneID, uint16 zoneType, uint8 *ieeeAddress, uint8 seqNum )
{
  uint8 *buf;
...
ZStatus_t zclSS_Send_IAS_WD_StartWarningCmd( uint8 srcEP, afAddrType_t *dstAddr,
                        zclCmdSSWDStartWarningPayload_t *warning, uint8 seqNum )
{
  uint8 buf[3];
...
ZStatus_t zclSS_Send_IAS_WD_SquawkCmd( uint8 srcEP, afAddrType_t *dstAddr,
                               zclCmdSSWDSquawkPayload_t *squawk, uint8 seqNum )
{
  uint8 buf[1];
 ...
 * @return  ZStatus_t
 */
static ZStatus_t zclSS_HdlIncoming( zclIncoming_t *pInMsg, uint16 logicalClusterID )
{
  ...
  if ( zcl_ClusterCmd( pInMsg->hdr.fc.type ) )
  {
    ...
    {
      stat = zclSS_HdlInSpecificCommands( pInMsg, logicalClusterID );
    }
    else
    {
...
static ZStatus_t zclSS_ProcessInZoneStatusCmdsClient( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )				
  {
    ...
    case COMMAND_SS_IAS_ZONE_STATUS_ENROLL_REQUEST:
      zclSS_ProcessInCmd_ZoneStatus_EnrollRequest( pInMsg );
      break;
...
static ZStatus_t zclSS_ProcessInACECmdsServer( zclIncoming_t *pInMsg )
{
  ...
  switch ( pInMsg->hdr.commandID )				
  {
    case COMMAND_SS_IAS_ACE_ARM:
      zclSS_ProcessInCmd_ACE_Arm( pInMsg );
      break;
    ...
    case COMMAND_SS_IAS_ACE_GET_ZONE_ID_MAP:
      zclSS_ProcessInCmd_ACE_GetZoneIDMap( pInMsg );
      break;

    case COMMAND_SS_IAS_ACE_GET_ZONE_INFORMATION:
      zclSS_ProcessInCmd_ACE_GetZoneInformation( pInMsg );
      break;
...
static void zclSS_ProcessInCmd_ZoneStatus_ChangeNotification( zclIncoming_t *pInMsg )
{
  ...
  if ( pCBs && pCBs->pfnChangeNotification )
    pCBs->pfnChangeNotification( pInMsg->pData[0], pInMsg->pData[1] );
...
static void zclSS_ProcessInCmd_ZoneStatus_EnrollRequest( zclIncoming_t *pInMsg )
{
  ...
  zclSS_IAS_Send_ZoneStatusEnrollResponseCmd( pInMsg->msg->endPoint, &(pInMsg->msg->srcAddr),
                                              status, zoneID, pInMsg->hdr.transSeqNum );
  if ( status == ZCL_STATUS_SUCCESS )
  {
    // Callback the application so it can fill in the Device IEEE Address  
    pCBs = zclSS_FindCallbacks( pInMsg->msg->endPoint );
    if ( pCBs && pCBs->pfnEnrollRequest )
      pCBs->pfnEnrollRequest( &(pInMsg->msg->srcAddr), zoneID, zoneType, manuCode );
  }
}
...
static void zclSS_ProcessInCmd_ZoneStatus_EnrollResponse(zclIncoming_t *pInMsg )
{
  ...
  if ( pCBs && pCBs->pfnEnrollResponse )
    pCBs->pfnEnrollResponse( pInMsg->pData[0], pInMsg->pData[1] );
}
#endif // ZCL_ZONE
...
static void zclSS_ProcessInCmd_ACE_Arm( zclIncoming_t *pInMsg )
{
  ...
  if ( pCBs && pCBs->pfnACE_Arm )
  {
    ...
    zclSS_Send_IAS_ACE_ArmResponse( pInMsg->msg->endPoint, &(pInMsg->msg->srcAddr),
                                    armNotification, pInMsg->hdr.transSeqNum );
  }
}
...
static void zclSS_ProcessInCmd_ACE_Bypass( zclIncoming_t *pInMsg )
{
  ...
  if ( pCBs && pCBs->pfnACE_Bypass )
    pCBs->pfnACE_Bypass( pInMsg->pData[0], &(pInMsg->pData[1]) ) ;
...
static void zclSS_ProcessInCmd_ACE_GetZoneIDMap( zclIncoming_t *pInMsg )
{
  ...
  zclSS_Send_IAS_ACE_GetZoneIDMapResponseCmd( pInMsg->msg->endPoint, &(pInMsg->msg->srcAddr),
                                              zoneIDMap, pInMsg->hdr.transSeqNum );
}
...
static void zclSS_ProcessInCmd_ACE_GetZoneInformation( zclIncoming_t *pInMsg )
{
  ...
  zclSS_Send_IAS_ACE_GetZoneInformationResponseCmd( pInMsg->msg->endPoint, &(pInMsg->msg->srcAddr),
                                                    ...
                                                    pZone->zoneAddress, pInMsg->hdr.transSeqNum );
}
...
static void zclSS_ProcessInCmd_ACE_GetZoneInformationResponse( zclIncoming_t *pInMsg )
{
  ...
  {
    zoneType = BUILD_UINT16( pInMsg->pData[1], pInMsg->pData[2] );
    pCBs->pfnACE_GetZoneInformationResponse( pInMsg->pData[0], zoneType, &(pInMsg->pData[3]) );
  }
}
#endif // ZCL_ACE
...
static void zclSS_ProcessInCmd_WD_StartWarning( zclIncoming_t *pInMsg )
{
  ...
  {
    warning.warningbyte = pInMsg->pData[0];
    duration = BUILD_UINT16( pInMsg->pData[1], pInMsg->pData[2] );
    pCBs->pfnWD_StartWarning( warning, duration );
  }
}
