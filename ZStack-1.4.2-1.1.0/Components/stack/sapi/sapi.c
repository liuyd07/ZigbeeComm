...
/******************************************************************************
    Filename:       sapi.c
    Revised:        $Date: 2007-05-22 09:15:27 -0700 (Tue, 22 May 2007) $
    Revision:       $Revision: 14384 $

    Description:

      Z-Stack Simple Application Interface.
...
void zb_BindDevice ( uint8 create, uint16 commandId, uint8 *pDestination )
{
  ...
  if ( create )
  {
    if (sapi_bindInProgress == 0xffff)
    {
      if ( pDestination )
      {
        // The source of the binding is this device
        source.addr.shortAddr = _NIB.nwkDevAddress;
        source.addrMode = Addr16Bit;

        destination.addrMode = Addr64Bit;
        osal_cpyExtAddr( destination.addr.extAddr, pDestination );

        ret = APSME_BindRequest( &source, sapi_epDesc.endPoint, commandId,
  ...
  else
  {
    ...
    while (pBind = bindFind(&source, sapi_epDesc.simpleDesc->EndPoint, commandId,
              FALSE, FALSE) )
    {
...
void zb_AllowBind ( uint8 timeout )
{
  ...
  osal_stop_timer( ZB_ALLOW_BIND_TIMER );
